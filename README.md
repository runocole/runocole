# Hello! I'm Cole Naomi 👋

## About Me
I'm a **Backend & AI/ML Engineer** passionate about building **scalable applications**, intelligent data solutions, and robust backend systems.  
I specialize in **Python, Django, Node.js, PostgreSQL**, and cloud deployments (AWS/Docker).  
I also enjoy exploring **machine learning workflows, data pipelines, and model deployment** to create data-driven solutions that solve real-world problems.

---

## Technical Skills

### Programming Languages
![Python](https://img.shields.io/badge/-Python-333?style=flat&logo=python) 
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=000)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat&logo=typescript&logoColor=fff)

### Frameworks & APIs
![Django](https://img.shields.io/badge/-Django-092E20?style=flat&logo=django) 
![Django REST Framework](https://img.shields.io/badge/-DRF-092E20?style=flat)
![FastAPI](https://img.shields.io/badge/-FastAPI-009688?style=flat&logo=fastapi) 
![React](https://img.shields.io/badge/-React-20232A?style=flat&logo=react&logoColor=61DAFB)

### Databases & Caching
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=flat&logo=postgresql)
![MySQL](https://img.shields.io/badge/-MySQL-00758F?style=flat&logo=mysql)
![Redis](https://img.shields.io/badge/-Redis-DC382D?style=flat&logo=redis)

### AI/ML & Data
![NumPy](https://img.shields.io/badge/-NumPy-013243?style=flat) 
![Pandas](https://img.shields.io/badge/-Pandas-150458?style=flat) 
![TensorFlow](https://img.shields.io/badge/-TensorFlow-FF6F00?style=flat)

### DevOps & Cloud
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat&logo=docker) 
![AWS](https://img.shields.io/badge/-AWS-232F3E?style=flat&logo=amazon-aws)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git)
![CI/CD](https://img.shields.io/badge/-CI%2FCD-007ACC?style=flat)

---

## 📂 Projects

### AI & Data Intelligence
- **[AI-Powered Business Intelligence Platform](https://github.com/runocole)**  
  Conversational AI chatbot, KPI dashboards, predictive analytics with **Python, FastAPI, Scikit-learn, XGBoost, PostgreSQL**.  

### Web & Full-Stack
- **[Smart Queue Management System](https://github.com/runocole/EasyPass-Backend)**  
  Real-time queue management with **Django, React, PostgreSQL, WebSockets**  
  [Live Demo](https://easypass-frontend.vercel.app/)

- **[Location-Based E-Commerce Platform](https://github.com/runocole)**  
  Geo-enabled shopping app with **Django, React, PostgreSQL, Geolocation APIs**

- **[Sports Scouting & Reporting System](https://github.com/runocole/bytecraft)**  
  Dynamic report generation, PDF exports, and real-time tracking using **Django REST Framework & React**

### Personal Portfolio
- **[Portfolio](https://runocole.netlify.app)** - Showcase of projects, skills, and experience

---

## 📊 GitHub Stats
![Cole's GitHub stats](https://github-readme-stats.vercel.app/api?username=runocole&show_icons=true&hide=issues,prs&theme=radical)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=runocole&layout=compact&theme=radical)

---

## 📫 Contact
[LinkedIn](https://www.linkedin.com/in/runocole) | [Email](mailto:runocole@gmail.com) | [Portfolio](https://runocole.netlify.app)
